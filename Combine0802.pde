...
static final int effectDuration = 18500;
...
static final float colorTransitionDuration = 7000;
...
static final float center_circleSpeed = 5;
...
static final int tunnelLifeTime = 50; // Frames
static int tunnelNumSegments = 4; // * 2
static float tunnelSegmentAngle = 90;
...
void draw() {
  ...
  if (millis() - lastEffectChangeTime > effectDuration) {
    ...
    currentColorIndex = int(random(0,24));
  ...
  // Change Color
  // if (millis() - colorTransitionStartTime >= colorTransitionDuration) {
...
void center() {
  ...
  for (int i = 0; i < 1; i++) {
    ...
    rotate(radians(centerAngle));
    beginShape();


    for (int j = 0; j < 30; j++) {
      float angle = center_angles[i] + j * center_angleStep;
      float x = radii[i] * cos(angle);
      float y = radii[i] * sin(angle);
      float amp = map(j, 0, 30, 0, radii[i]);

      // Modify the amp value to make the object expand and contract
      float expansionFactor = 1.5; // Adjust this value for the desired expansion/contraction effect
      float expandedAmp = amp * expansionFactor;
      
  
      ellipse(x, y, expandedAmp, expandedAmp);
...
void moving_spiral(){
  ...
  for (int grad = 0; grad < 3600; grad += 12) {
    ...
    float x = 960 + cos(angle) * radius;
    float y = 540 + sin(angle) * radius;
    ellipse(x, y, 7, 7);
    radius += 2;
  }
}
